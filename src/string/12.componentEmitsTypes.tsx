...
type ComponentEmitsType<T> = T extends Record<string, any>
  ? {
      [K in keyof T as `on${CamelCase<K extends string ? K : "">}`]: T[K] extends (...args: [...infer A]) => boolean
        ? (...args: [...A]) => void
        : never;
    }
  : never;

type C = ComponentEmitsType<source>;
function createComponent<Emits extends Record<string, any>>(emits: Emits): ComponentEmitsType<Emits> {
  return [{ emits }] as any;
}
