...
// export type CamelCase<T extends string, Result extends string = ""> = T extends `${infer L}-${infer R}`
//   ? CamelCase<R, `${Result}`>
//   : `${Result}${CapitalizeString<T>}`;

// 解法2. 遍历的时候老方法，
export type CamelCase<T extends string, Result extends string = ""> = T extends `${infer L}-${infer R1}${infer R2}`
